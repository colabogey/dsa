...
You are required to answer Q queries of type [u, c], where u is an integer and c
is a lowercase letter. The query result is the number of nodes in the subtree of
node u containing c. Signature int[] countOfNodes(Node root, ArrayList<Query>
queries, String s) Input A pointer to the root node, an array list containing Q
queries of type [u, c], and a string s Constraints N and Q are the integers
between 1 and 1,000,000 u is a unique integer between 1 and N s is of the length
of N, containing only lowercase letters c is a lowercase letter contained in
string s Node 1 is the root of the tree Output An integer array containing the
response to each query Example
...
letter of the string 'b', Node 3 corresponds to third letter of the string 'a'.
output = [2]
Both Node 1 and Node 3 contain 'a', so the number of nodes within the subtree of
Node 1 containing 'a' is 2.
...
Node* findNode(Node* root, int val) {
    ...
    for(int i = 0; i< root->children.size(); i++) {
        return findNode(root->children[i], val);
    }
}

int countChildVals(Node* startNode, std::string& s, char c) {
    // if s[node->val] == c -> count++`
    ...
    for(int i = 0; i< startNode->children.size(); i++) {
        int idx = startNode->children[i]->val - 1;
        if(s[idx] ==  c) {
            count++;
        }
    }
    ...
    for(int i = 0; i< startNode->children.size(); i++) {
        count += countChildVals(startNode->children[i], s, c);;
    }
...
vector<int> countOfNodes(Node *root, vector<Query> queries, string s) {
    ...
    vector<int> ans;
    for (Query q : queries) {
        Node* startNode = findNode(root, q.u);
        if(startNode == nullptr) {
            return ans;
        }
        int count = 0;
        int idx = startNode->val - 1;
        if(s[idx] == q.c) {
            count++;
        }
        count += countChildVals(startNode, s, q.c);
        ans.push_back(count);
    }
    return ans;
}
